...
const aad = new TextEncoder().encode("oink-keystore-v1|pascal|public-key");
...
test("rejects an AAD swap", async () => {
  const blob = await seal(encKey, entropy, aad);
  await expect(open(encKey, blob, new TextEncoder().encode("oink-keystore-v1|other|public-key"))).rejects.toThrow();
});
